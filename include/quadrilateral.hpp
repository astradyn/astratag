...
class QuadrilateralDetector{
	...
	private:
		...
		cv::Mat applyAdaptiveThreshold(const cv::Mat& gradientMagnitude);                std::vector<std::vector<cv::Point>> findContours(const cv::Mat& binary);
...
void processImagesBatch(const std::string& inputFolder, const std::string& outputFolder);
